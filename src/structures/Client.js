...
class Client extends EventEmitter {
  ...
  constructor(options) {
     ...
     */
    this.totp_two_factor_on = null;

    /**
     * @type {string}
     * Identifier of the two factor authentication.
     */
    this.two_factor_identifier = null;
              ...
              if (isInboxThreadPath) {
                ...
                if (this.cache.chats.has(threadID)) {
                  ...
                  if (oldChat.users.size < chat.users.size) {
                    const userAdded = chat.users.find(
                      (u) => !oldChat.users.has(u.id)
                    );
                    if (userAdded) this.emit("chatUserAdd", chat, userAdded);
                  } else if (oldChat.users.size > chat.users.size) {
                    const userRemoved = oldChat.users.find(
                      (u) => !chat.users.has(u.id)
                    );
                    if (userRemoved)
                      this.emit("chatUserRemove", chat, userRemoved);
                  }
              ...
              if (isMessagePath) {
                ...
                this.fetchChat(threadID).then((chat) => {
                  ...
                  if (chat.messages.has(messagePayload.item_id)) {
                    ...
                    const oldMessage = Object.assign(
                      Object.create(message),
                      message
                    );
                    chat.messages
                      .get(messagePayload.item_id)
                      ._patch(messagePayload);
            ...
            case "add": {
              ...
              if (isMessagePath) {
                ...
                this.fetchChat(threadID).then((chat) => {
                  ...
                  const messagePayload = JSON.parse(data.value);
                  if (
                    messagePayload.item_type === "action_log" ||
                    messagePayload.item_type === "video_call_event"
                  )
                    return;
  ...
  async login(username, password, state) {
    ...
    try {
      ...
      console.log("login succeed on the first try");
      this.postLogin(ig, response);
      return { status: "ok" };
    } catch (err) {
      ...
      if (err.toString().indexOf("IgLoginTwoFactorRequiredError") !== -1) {
        ...
        console.log("general error (maybe login or password incorrect?)");
  ...
  async postLogin(ig, response) {
    ...
    const userData = await ig.user.info(
      response.pk || response.logged_in_user.pk
    );
